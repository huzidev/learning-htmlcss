...
let d =  document
console.log(d); //BY DOING THIS METHOD WE WILL GET A (#DOCUMENT) ELEMENT INSIDE OF THE CONSOLE OF WEB PAGE AND IF WE CLICK ON THAT ELEMENT WE'LL GET THE LAYOUT OF OURS HTML PAGE WHICH IS A DOM (DOCUMENT OBJECT MODEL)
...
// BY THIS METHOD WE CAN ALL THE ELEMENTS FROM HTML TO OURS CONSOLE PORTION OF WEBPAGE
// ARRAY.FROM(D) IS BASICALLY MEANS THAT MADE AN ARRAY FROM (D) BECAUSE IF WE JUST WRITE
...
// THIS FUNCTION IS IMPORTANT AS WE HAVE TO EXTRACT SEPCIFIC LINKS AND WE JUST HAVE TO WRITE DOWN THIS FUNCTION SO WE CAN SEE ALL THE LINKS USED IN OURS HTML
...
//WE CAN WRITE ALERT FUNCTION LIKE THAT AND BY JUST SIMPLE METHOD BECAUSE WINDOW IS A GLOBAL ELEMENT AND WE DONT HAVE TO PUT IT AGAIN AND AGAIN GLOBAL MEANS IT IS ALL OVER THE PLACE

// INNER AND OUTER WIDTH

d = innerHeight //INNER HEIGHT THE HEIGHT OF OURS WEBPAGE AND INNER HEIGHT IS BASICALLY THE VIEW PORT HEIGHT OF OURS PAGE WHILE OUTER HEIGHT IS THE BORDER TO BORDER HEIGHT
console.log(d);

d = innerWidth //INNER WIDTH THE WIDTH OF OURS WEBPAGE ONE MORE NO MATTER WHICH WIDTH IS IT INNER OR OUTER IT DOESN'T TAKE SCROOL BAR
console.log(d);
...
d = location.toString() //WILL PROVIDE THE URL OF YOURS LOCAL HOST
console.log(d);
...
function scrollWindow() {
    ...
    myWindow.focus(); //BY USING FOCUS WINDOW WE'LL HAVE THE FOCUS ON OUR NEW WINDOW BAR MEANS IT'LL REMAINS AT FRONT PAGE
}
...
function resizeTo(){
    myWindow.resizeTo(800, 800);   //FIRST ONE IS REFERING TO WIDTH AND SECOND ONE IS FOR HEIGHT (RESIZETO) FUNCTION JUST RESIZE AS IT IS AS MUCH WE PROVIDE THE DATA
    myWindow.focus();
}

function resizeBy() {
    myWindow.resizeBy(500, 500);  //WHAT (RESIZEBY) ACTUALLY IS THAT IT RESIEZE YOURS WIDTH AND HEIGHT SPECIFIALLY WHAT WE PROVIDE BUT ALSE IT PLUS (+) THE WIDTH AND HEIGHT FROM THE WINDOW OPEN FUNCTION FOR INSATNCE WE PROVIDE 500 WIDTH AND 500 HEIGHT AND WE ALREADY HAVE GIVEN 200 PX OF HEIGHT AND WIDTH ABOVE AT WINDOW OPEN THEREFORE IT SUM UP ALL THESE AND PROVIDE US THE RESULT
    myWindow.focus(); 
}
...
function scrollWindows() {
    window.scrollTo(0, 50); // (SCROLLTO) WILL WORKS AS ABSOLUTE MANNER MEANS IT WOULD LEAD YOURS PAGE TO TOP IF WE GAVE IT (O, O) MEANS 0PX FORM X-AXIS AND 0PX FROM Y-AXIS SINCE WE GAVE 50 FROM Y-AXIS THIS MEANS IS THAT IT WILL LEAD OURS PAGE TO TOP BUT FROM 50PX FROM TOP 
}
...
function go() {
    history.go(2); // WILL LEAD US TO FORWARD OR BACKWARD DEPENDS ON THAT WHICH SIGN WE'VE GIVEN TO IT IF WE GAVE (+VE) SIGN IT'LL GO FORWARD & IF WE GAVE (-VE) SIGN IT'LL GO BACKWARD, IT DEPENDS HOW MANY NUMBERS WE ADDED LIKE IF WE ADD JUST 2 (WHICH MEANS +2) SO IT'LL JUMP 2 TABS FORWARD 
}